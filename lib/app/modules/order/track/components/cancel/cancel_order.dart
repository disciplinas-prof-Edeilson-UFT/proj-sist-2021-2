...
import 'package:pscomidas/app/modules/order/track/components/cancel/class/cancel_checked.dart';
      ...
      onPressed: () => showDialog(
          ...
          builder: (BuildContext context) {
            return SingleChildScrollView(
              child: AlertDialog(
                title: const Text('Cancelar padido', style: TextStyle(fontFamily: 'Nunito')),
                content: Column(
                  children: [
                    Observer(builder: (_) {
                        return _checkList('Errei a forma de pagamento', controller.payments);
                    },),
                    Observer(builder: (_) {
                        return _checkList('Endereço está incorreto', controller.adress);
                    },),
                    Observer(builder: (_) {
                        return _checkList('Prato errado ou item faltando', controller.prato_errado);
                    },),
                    Observer(builder: (_) {
                        return _checkList('Comprei sem querer', controller.compra_errada);
                    },),
                    Observer(builder: (_) {
                        return _checkList('Horário de entrega é muito tarde', controller.horario);
                    },),
                    Observer(builder: (_) {
                ...
                actions: [
                  ...
                  Observer(builder: (_) {
                     return controller.confirm() ? _buttonConfirm() : Container();
                  })
                ],
              ),
            );
